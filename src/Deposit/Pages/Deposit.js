...
import SucessDiv from '../SucessDiv';
const Deposit = (props) => {
  const initailState = {
    name: {
      value: "",
          ...
          switch (action.type) {
            ...
            case "submit":
              return {
                ...state,
                success: true,
                balance: dep(state.name.value) + state.balance,
              };
            case "reset":
               return initailState;
            case "error":
    ...
    if (state.name.error || state.comment.error) {
      ...
      if (state.name.value === "" || state.comment.value === "") {
       error()
        ...
        let date = giveDate();
        props.addData(state.name.value, state.comment.value, "Deposit", date);
      }
    ...
    >
      ...
      {state.success && (
        <SucessDiv
          ...
          msg=" Your amount has deposited successfully."
          onClick={back}
          header="deposit"
        />
      )}
